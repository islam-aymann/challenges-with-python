...
class Solution:
    def threeSum1(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in itertools.combinations(nums, 3):
            ...
            result_set = set(i)
            if not sum(i) and result_set not in result_sets:
        ...
        for i in range(ln):
            for j in range(i + 1, ln):
                try:
                    ...
                    visiting = {nums[i], nums[j], nums[k]}
                    if (visiting not in visited) and (
                            i != j and i != k and j != k):
    ...
    def threeSum5(self, nums: List[int]) -> List[List[int]]:
        result = []
        nums.sort()
...
if __name__ == "__main__":
    ...
    print(
        1000
        * timeit.timeit(
            f"Solution().threeSum({nums})", number=10000,
            globals=globals()
        ),
        "ms",
    )
